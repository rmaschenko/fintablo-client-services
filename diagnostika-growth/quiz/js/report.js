...
(function () {
  ...
  function renderWarmIcpBlock() {
    ...
    return (
      ...
      '<h2>Подключите Финтабло на&nbsp;7 дней&nbsp;— сводим ДДС, ОПиУ и&nbsp;учёт по&nbsp;направлениям из&nbsp;вашей банковской выписки</h2>' +
      '<p>Загружаете банковскую выписку, открываете шаблон под вашу отрасль&nbsp;— за&nbsp;день видите ДДС, ОПиУ и&nbsp;прибыль по&nbsp;направлениям. Работает&nbsp;— остаётесь, не&nbsp;работает&nbsp;— никаких обязательств.</p>' +
      '<div class="dg-final-checklist">' + items.map(checkItem).join('') + '</div>' +
