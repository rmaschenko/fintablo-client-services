...
(function () {
  ...
  // title = что собственник реально получает; proofPoint = бизнес-эффект, не процесс.
  const RECOMMENDATIONS = {
    margin_blind: {
      title: 'Видеть прибыль и убыток по каждому направлению',
      body: 'Маржа по проектам, направлениям или объектам в моменте — а не на годовом итоге, когда уже ничего не вернуть. Решение закрыть убыточное направление или вложить в прибыльное — на основе цифры, а не интуиции.',
      whyHard: 'Самостоятельно собрать такой расчёт в Excel — 2-3 недели работы финансиста на настройку и 4-5 дней каждый месяц на закрытие. К моменту, когда отчёт готов, прошедший месяц уже не вернуть.',
      howFintablo: 'Маржа по контурам собирается из интеграций с банками и 1С — обновляется каждый день. Настройка под ваш профиль занимает 1-2 встречи с финансовым экспертом Финтабло.',
      proofPoint: 'Решение «закрыть или растить» — с цифрой в руках, не на ощупь',
      tags: ['маржа по направлениям', 'разбивка по контурам', 'ОПиУ'],
      visual: 'margin'
    },
    cash_surprise: {
      title: 'Видеть приближение кассового разрыва за недели до события',
    ...
    manual_close: {
      title: 'Освободить 18-25 часов финансиста в месяц для аналитики',
    ...
    blind_decisions: {
      title: 'Опирать решения по найму и проектам на цифры, а не на ощущения',
